...
如果你是 **Claude Code 初学者**，可以把 **GStack** 当成一个很好的入门辅助层。它提供结构化角色和固定的认知入口，能明显降低“面对空白提示不知道怎么开口”的成本。

### 为什么这一层对新手有帮助？

很多人第一次用 Claude Code 时，卡住的不是命令本身，而是：

1. 不知道如何开始，提示词过于空白
2. 缺乏结构化思维，不知道应该先问什么
3. 没有角色意识，习惯把 AI 当成一个泛化助手
4. 输出不稳定，容易得到泛泛而谈的回答

GStack 的价值在于先给你一套“角色化入口”，让你用更稳定的方式开始协作，而不是每次都从零组织思路。
...
1. **`/office-hours`** - 产品办公室时间，适合重新思考需求和方向
2. **`/plan-ceo-review`** - CEO / 创始人视角，适合做方向判断和优先级取舍
3. **`/plan-eng-review`** - 工程经理视角，适合设计方案和拆解实现路径
4. **`/review`** - 高级工程师视角，适合代码审查和风险检查
5. **`/qa`** - QA 负责人视角，适合测试应用和覆盖边界场景
6. **`/ship`** - 发布工程师视角，适合整理发布前检查和交付动作

如果你想先理解 GStack 的角色体系，再决定是否纳入自己的 Claude Code 工作流，可以先看上游 [GStack 仓库](https://github.com/garrytan/gstack)。
...
11. **[长期在线助理系统 + Claude CLI 集成指南](docs/ASSISTANT_CLAUDE_INTEGRATION_CN.md)** — 讲清楚任何长期在线助理、bot、提醒系统或控制层，如何把任务交给 Claude CLI 仓库工作流，以及 MCP 到底该共享到哪一层。
...
3. 如果你容易卡在“应该怎么问”，先补看上游 [GStack 仓库](https://github.com/garrytan/gstack)
4. 在真实项目里做几个小任务
5. 读 [HOW_TO_START_EXISTING_PROJECT_CN.md](HOW_TO_START_EXISTING_PROJECT_CN.md)
6. 先只安装本仓库里的一个组件，不要一口气全装
...
3. **长期在线助理外环 + Claude CLI 内环**：看 [docs/ASSISTANT_CLAUDE_INTEGRATION_CN.md](docs/ASSISTANT_CLAUDE_INTEGRATION_CN.md)
...
5. 安装 **Doc Scanner**
6. 安装 **Status Line**
7. 如果你是 Claude Code 新手，先补看一次 GStack 角色体系，再决定是否纳入日常流程
8. 只有在你使用 Pencil 时才安装 **Design Context**
